...
export class PolygonStrategy implements DrawingStrategy {
   ...
   * 繪製中（滑鼠移動，用於預覽）
   */
  onMove(point: Point, context: DrawingContext, tempCanvas?: HTMLCanvasElement) {
    if (this.state !== PolygonState.DRAWING) return
  ...
  onEnd(point: Point | null, context: DrawingContext) {
    console.log('PolygonStrategy.onEnd', { point, pointsCount: this.points.length })
   ...
   */
  onCancel() {
    console.log('PolygonStrategy.onCancel')
   ...
   * 獲得多邊形邊界
   */
  private getPolygonBounds(): { minX: number; minY: number; maxX: number; maxY: number } {
